...
import { atomCryptoWithdrawalHistoryForm } from "utils/store";
...
import cryptoWithdrawalHistoryService from "./service";

const CryptoWithdrawalHistory = () => {
  const [form, setForm] = useAtom(atomCryptoWithdrawalHistoryForm);
  ...
  const list = useRequest(cryptoWithdrawalHistoryService.list, {
    manual: true,
    onError: (err) =>
      notification.error({
        message: err.message,
  ...
  return (
    <div className="flex flex-col gap-4">
      <Breadcrumbs path={<span>Withdrawal history</span>} />
      <PageCard xR>
        <div className="px-2">
          <InitTableHeader
            setSearch={(e) => {
            ...
            customHeaderTitle="Withdrawal history"
            refresh={() =>
...
export default CryptoWithdrawalHistory;
